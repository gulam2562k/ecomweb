...
export default function Shop() {
  ...
  function postPriceFilter(e) {
    e.preventDefault();
    var p = [];
    if (mc === "All" && sc === "All" && br === "All") p = allProductStateData;
  ...
  function filterProducts(mc, sc, br) {
    if (mc === "All" && sc === "All" && br === "All")
      setProducts(allProductStateData);
    else if (mc !== "All" && sc === "All" && br === "All")
      setProducts(
        allProductStateData.filter((item) => item.maincategory === mc)
      );
    else if (mc === "All" && sc !== "All" && br === "All")
      setProducts(
        allProductStateData.filter((item) => item.subcategory === sc)
      );
    else if (mc === "All" && sc === "All" && br !== "All")
      setProducts(allProductStateData.filter((item) => item.brand === br));
    else if (mc !== "All" && sc !== "All" && br === "All")
      setProducts(
        allProductStateData.filter(
          (item) => item.maincategory === mc && item.subcategory === sc
  ...
  function sortFilter(e) {
    ...
    if (value === "1") setProducts(products.sort((x, y) => y.id - x.id));
    else if (value === "2")
    ...
    <>
      ...
      <div className="product-section my-5">
        <div className="container">
          <div className="row">
            <div className="col-lg-2 scrollable">
              <div className="list-group mb-3">
                <a href="#" className="list-group-item  menu-bg text-light">
                  Maincategory
                </a>
                <button
                  onClick={() => selectCategory("All", sc, br)}
                  className=" btn list-group-item list-group-item-action"
                >
                  All
                </button>
                {allMaincategoryStateData &&
                  allMaincategoryStateData.map((item, index) => {
                    return (
                      <button
                        key={index}
                        onClick={() => selectCategory(item.name, sc, br)}
                        className=" btn list-group-item list-group-item-action"
                      >
                        {item.name}
                      </button>
              ...
              <div className="list-group mb-3">
                ...
                <button
                  onClick={() => selectCategory(mc, "All", br)}
                  className=" btn list-group-item list-group-item-action"
                >
                  All
                </button>
                {allSubcategoryStateData &&
                  allSubcategoryStateData.map((item, index) => {
                    return (
                      <button
                        key={index}
                        onClick={() => selectCategory(mc, item.name, br)}
                        className=" btn list-group-item list-group-item-action"
                      >
                        {item.name}
                      </button>
              ...
              <div className="list-group mb-3">
                ...
                <button
                  onClick={() => selectCategory(mc, sc, "All")}
                  className=" btn list-group-item list-group-item-action"
                >
                  All
                </button>
                {allBrandStateData &&
                  allBrandStateData.map((item, index) => {
                    return (
                      <button
                        key={index}
                        onClick={() => selectCategory(mc, sc, item.name)}
                        className=" btn list-group-item list-group-item-action"
                      >
                        {item.name}
                      </button>
            ...
            <div className="col-lg-10">
              <div className="btn-group w-100 mb-3">
                <input
                  ...
                  value={search}
                />
                <button className="btn btn-primary success" onClick={postSeach}>
                  Search
                </button>
              </div>
